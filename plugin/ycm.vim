...
let g:ycm_min_num_of_chars_for_completion = 4                                               "输入第2个字符开始补全
...
let g:UltiSnipsSnippetDirectories=["~/vim/Bundles/vim-snippets/UltiSnips"]                  " 定义存放代码片段的文件夹，使用自定义和默认的，将会的到全局，有冲突的会提示
...
" let g:UltiSnipsJumpBackwardTrigger="<S-Tab>"
" end
" UltiSnips completion function that tries to expand a snippet. If there's no snippet for expanding, it checks for completion window and if it's shown, selects first element. If there's no completion
" window it tries to jump to next placeholder. If there's no placeholder it just returns TAB key
